...
class PropertySearchService:
    ...
    def search_properties(
        query: str, limit: int = 5, current_user: Optional[User] = None
    ) -> List[Property]:
        """
        ...
        queryset = Property.objects.filter(q_objects, is_verified=True)
        ...
        properties = queryset.select_related("seller").prefetch_related("images")[
            :limit
        ]
...
class ChatService:
    ...
    def create_or_get_session(self, user: User) -> ChatSession:
        ...
        session = (
            ChatSession.objects.filter(user=user, is_active=True)
            .order_by("-created_at")
            .first()
        )
...
---"""
    ...
    def generate_response(
        ...
        properties = self.property_search.search_properties(
            user_message, current_user=session.user
        )
        ...
        recent_messages = ChatMessage.objects.filter(session=session).order_by(
            "-created_at"
        )[:5]
...
Current conversation context: You are helping a buyer find their ideal property."""
        ...
        try:
            ...
            ai_response = getattr(gemini_resp, "text", None) or (
                ...
                if getattr(gemini_resp, "candidates", None)
                else ""
            )
    ...
    def generate_response_for_guest(
        self, message: str, history: List[Dict]
    ) -> Dict[str, Any]:
        """Generate AI response for guest users (no DB persistence)"""

        # Search for relevant properties (no user context)
        properties = self.property_search.search_properties(message, current_user=None)
...
Current conversation context: You are helping a guest user find their ideal property."""
